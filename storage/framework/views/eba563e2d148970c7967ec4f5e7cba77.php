...
<div class="p-6">
    ...
    <div class="bg-white rounded-xl shadow-md p-6">
        <form action="<?php echo e(route('informasi.update', $informasi->informasi_id)); ?>" method="POST">
            <?php echo csrf_field(); ?>
            ...
            <div class="mb-4">
                ...
                <div id="donasi-list">
                    ...
                    <?php if(count($donasiList) > 0): ?>
                        <?php $__currentLoopData = $donasiList; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $donasi): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <div class="donasi-item flex flex-wrap gap-2 mb-2 items-center">
                            <input type="text" name="donasi_nama[]" value="<?php echo e($donasi['nama'] ?? ''); ?>" 
                                   class="flex-1 border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500" 
                                   placeholder="Nama barang" style="min-width: 150px;">
                            <input type="text" name="donasi_jumlah[]" value="<?php echo e($donasi['target'] ?? $donasi['jumlah'] ?? 0); ?>" 
                            ...
                            <div class="flex gap-2">
                                <select name="donasi_satuan[]" class="satuan-select border border-gray-300 rounded-lg px-3 py-2">
                                    <option value="kg" <?php echo e(($donasi['satuan'] ?? '') == 'kg' ? 'selected' : ''); ?>>Kg</option>
                                    <option value="liter" <?php echo e(($donasi['satuan'] ?? '') == 'liter' ? 'selected' : ''); ?>>Liter</option>
                                    <option value="paket" <?php echo e(($donasi['satuan'] ?? '') == 'paket' ? 'selected' : ''); ?>>Paket</option>
                                    <option value="unit" <?php echo e(($donasi['satuan'] ?? '') == 'unit' ? 'selected' : ''); ?>>Unit</option>
                                    <option value="lainnya" <?php echo e(!in_array($donasi['satuan'] ?? '', ['kg','liter','paket','unit']) && !empty($donasi['satuan']) ? 'selected' : ''); ?>>Lainnya...</option>
                                </select>
                                <input type="text" name="donasi_satuan_lainnya[]" 
                                       value="<?php echo e(!in_array($donasi['satuan'] ?? '', ['kg','liter','paket','unit']) ? ($donasi['satuan'] ?? '') : ''); ?>" 
                                       class="satuan-lainnya border border-gray-300 rounded-lg px-3 py-2" 
                                       placeholder="Satuan lain"
                                       style="width: 100px; display: <?php echo e(!in_array($donasi['satuan'] ?? '', ['kg','liter','paket','unit']) && !empty($donasi['satuan']) ? 'inline-block' : 'none'); ?>;">
                            </div>
            ...
            <div class="mb-4">
                ...
                <select name="status_kolaborasi" class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500">
                    <option value="dibuka" <?php echo e(($informasi->status_kolaborasi ?? '') == 'dibuka' ? 'selected' : ''); ?>>Dibuka</option>
                    <option value="ditutup" <?php echo e(($informasi->status_kolaborasi ?? '') == 'ditutup' ? 'selected' : ''); ?>>Ditutup</option>
                </select>
            </div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Sirela\resources\views/informasi/edit.blade.php ENDPATH**/ ?>