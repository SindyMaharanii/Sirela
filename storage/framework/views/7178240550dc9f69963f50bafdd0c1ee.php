...
<div class="p-6">
    <div class="max-w-5xl mx-auto">
        <a href="<?php echo e(route('lembaga.show', $informasi->lembaga_id)); ?>" class="text-blue-600 mb-4 inline-block">← Kembali ke Profil Lembaga</a>
        ...
        <div class="bg-white rounded-xl shadow-md overflow-hidden mb-6">
            <div class="bg-gradient-to-r from-teal-500 to-emerald-600 px-6 py-4 flex justify-between items-center">
                ...
                <a href="<?php echo e(route('informasi.edit', $informasi->informasi_id)); ?>" 
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                    <div class="bg-blue-50 rounded-xl p-4 text-center">
                        <p class="text-3xl font-bold text-blue-600"><?php echo e($informasi->jumlah_anak_asuh ?? 0); ?></p>
                        <p class="text-gray-600 text-sm">Jumlah Anak Asuh</p>
                    </div>
                    <div class="bg-emerald-50 rounded-xl p-4 text-center">
                        <p class="text-3xl font-bold text-emerald-600"><?php echo e($informasi->rentang_usia ?? '-'); ?></p>
                        <p class="text-gray-600 text-sm">Rentang Usia</p>
                    </div>
                    <div class="bg-purple-50 rounded-xl p-4 text-center">
                        <p class="font-bold text-purple-600"><?php echo e(\Carbon\Carbon::parse($informasi->tanggal_update ?? now())->format('d M Y H:i')); ?></p>
                        <p class="text-gray-600 text-sm">Terakhir Update</p>
                    </div>
                </div>
                
                <!-- Status Kolaborasi -->
                <?php if($informasi->status_kolaborasi == 'dibuka'): ?>
                ...
                <?php if($informasi->profil_anak): ?>
                <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                    <p class="font-semibold">Deskripsi Anak Asuh:</p>
                    <p><?php echo e($informasi->profil_anak); ?></p>
                </div>
                <?php endif; ?>
            </div>
        </div>
                            ...
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="border border-gray-300 px-4 py-2 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button onclick="bukaModalEdit('<?php echo e($index); ?>')" 
                                                class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-lg text-sm transition">
                                            <i class="fas fa-edit"></i> Edit
                                        </button>
                                        <button onclick="hapusKebutuhan('<?php echo e($index); ?>', '<?php echo e(addslashes($item['nama'])); ?>')" 
...
<script>
    let currentList = <?php echo json_encode($list, 15, 512) ?>;
    let informasiId = <?php echo e($informasi->informasi_id); ?>;
    ...
    async function simpanKeServer() {
        const response = await fetch('<?php echo e(route("informasi.kebutuhan.update", $informasi->informasi_id)); ?>', {
            method: 'POST',
            headers: {
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\Sirela\resources\views\informasi\show.blade.php ENDPATH**/ ?>